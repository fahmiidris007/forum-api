  ...
  async getReplyById(commentId) {
    const query = {
      text: `SELECT replies.id, users.username, replies.date,
            replies.content, replies.is_deleted AS "isDeleted" FROM replies
            LEFT JOIN users ON replies.owner = users.id WHERE comment_id = $1
            GROUP BY replies.id, users.username ORDER BY date`,
      values: [commentId],
    };
    ...
    return result.rows.map((value) => new DetailReply(value));
  }
}
